...
actual object Crypto {
    ...
    actual fun encodeBase64(data: ByteArray): String {
        ...
        return wasmBtoa(binaryString)
    }

    actual fun decodeBase64(data: String): ByteArray {
        if (data.isEmpty()) return ByteArray(0)
        val binaryString = wasmAtob(data)
    ...
    actual fun inflate(data: ByteArray): ByteArray {
        if (data.isEmpty()) return ByteArray(0)
        val base64Input = encodeBase64(data)
        val base64Output = wasmPakoInflate(base64Input)
    ...
    private fun jsBigIntModPow(base: String, exponent: String, modulus: String): String {
        return wasmBigIntModPow(base, exponent, modulus)
    }
...
""")
private external fun wasmBigIntModPow(base: String, exponent: String, modulus: String): String
